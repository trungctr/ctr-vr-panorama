import { Color } from '../thre146/build/three.module.js'
...
class CanvasColorPicker {
	...
	createGradients(ctx) {
		this.grdWhite = ctx.createLinearGradient(
			this.picker.x,
			0,
			this.picker.x + this.picker.width,
			0
		)
		this.grdWhite.addColorStop(0, 'rgba(255,255,255,1)')
		this.grdWhite.addColorStop(1, 'rgba(255,255,255,0)')

		this.grdBlack = ctx.createLinearGradient(
			0,
			this.picker.y,
			0,
			this.picker.y + this.picker.height
		)
		...
		const grd = ctx.createLinearGradient(
			...
			this.strip.y,
			0,
			this.strip.y + this.strip.height
		)

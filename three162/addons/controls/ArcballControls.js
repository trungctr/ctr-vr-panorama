				...
				case STATE.ROTATE:
					if (this.enableRotate) {
						if (restart) {
							...
							const amount =
								Math.max(distance / this._tbRadius, angle) *
								this.rotateSpeed //effective rotation angle
					...
					if (this.enableZoom) {
						if (restart) {
							...
							if (movement < 0) {
								size =
									1 /
									Math.pow(this.scaleFactor, -movement * screenNotches)
							} else if (movement > 0) {
								size = Math.pow(
									this.scaleFactor,
									movement * screenNotches
								)
							}
				...
				case STATE.FOV:
					if (this.enableZoom && this.camera.isPerspectiveCamera) {
						if (restart) {
							...
							const y =
								x * Math.tan(MathUtils.DEG2RAD * this._fovState * 0.5)
			...
			switch (mouseOp) {
				...
				case 'FOV':
					if (this.camera.isPerspectiveCamera) {
						...
						const y =
							x * Math.tan(MathUtils.DEG2RAD * this.camera.fov * 0.5)

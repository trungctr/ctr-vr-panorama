...
const MXElements = [
	...
	new MXElement('worleynoise2d', mx_worley_noise_float, [
		'texcoord',
		'jitter'
	]),
	new MXElement('worleynoise3d', mx_worley_noise_float, [
		'texcoord',
		'jitter'
	]),
...
class MaterialXNode {
	...
	toPhysicalMaterial() {
		...
		for (const nodeX of this.children) {
			const shaderProperties = this.materialX.getMaterialXNode(
				nodeX.nodeName
			)
...
class MaterialX {
	...
	parseNode(nodeXML, nodePath = '') {
		...
		for (const childNodeXML of nodeXML.children) {
			const childMXNode = this.parseNode(
				childNodeXML,
				materialXNode.nodePath
			)

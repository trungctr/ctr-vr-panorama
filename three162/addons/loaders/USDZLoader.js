			...
			for (const name in data) {
				...
				if (name.startsWith('def Mesh')) {
					...
					if ('texCoord2f[] primvars:st' in data) {
						object['texCoord2f[] primvars:st'] =
							data['texCoord2f[] primvars:st']
					}
			...
			if (data !== undefined) {
				if ('def Shader "PreviewSurface"' in data) {
					...
					if ('color3f inputs:diffuseColor.connect' in surface) {
						...
						const sampler = findTexture(
							root,
							/(\w+).output/.exec(path)[1]
						)
					...
					if ('float inputs:roughness.connect' in surface) {
						...
						material.roughness = parseFloat(
							surface['float inputs:roughness']
						)
					}
					...
					if ('float inputs:metallic.connect' in surface) {
						...
						material.metalness = parseFloat(
							surface['float inputs:metallic']
						)
					}
					...
					if ('float inputs:clearcoat.connect' in surface) {
						...
						material.clearcoat = parseFloat(
							surface['float inputs:clearcoat']
						)
					}

					if ('float inputs:clearcoatRoughness.connect' in surface) {
						const path =
							surface['float inputs:clearcoatRoughness.connect']
						const sampler = findTexture(
							root,
							/(\w+).output/.exec(path)[1]
						)

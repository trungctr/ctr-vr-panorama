...
class KTX2Loader extends Loader {
	...
	async detectSupportAsync(renderer) {
		this.workerConfig = {
			astcSupported: await renderer.hasFeatureAsync(
				'texture-compression-astc'
			),
			etc1Supported: await renderer.hasFeatureAsync(
				'texture-compression-etc1'
			),
			etc2Supported: await renderer.hasFeatureAsync(
				'texture-compression-etc2'
			),
			dxtSupported: await renderer.hasFeatureAsync('texture-compression-bc'),
			bptcSupported: await renderer.hasFeatureAsync(
			...
			for (let mip = 0; mip < levelCount; mip++) {
				...
				for (let layer = 0; layer < layerCount; layer++) {
					...
					if (
						...
						mip === 0 &&
						layer === 0 &&
						(levelInfo.origWidth % 4 !== 0 ||
							levelInfo.origHeight % 4 !== 0)
					) {
						console.warn(
...
async function createRawTexture(container) {
	...
	for (
		let levelIndex = 0;
		levelIndex < container.levels.length;
		levelIndex++
	) {

...
class WebGLNodeBuilder extends NodeBuilder {
	...
	_parseObject() {
		...
		if (
			...
			renderer.toneMappingNode.isNode === true
		) {
			this.addSlot(
				'fragment',
				new SlotNode({
		...
		if (material.colorNode && material.colorNode.isNode) {
			this.addSlot(
				...
				new SlotNode({
					...
					target:
						'vec4 diffuseColor = %RESULT%; diffuseColor.a *= opacity;'
				})
			)
		}
		...
		if (material.isMeshStandardNodeMaterial) {
			...
			if (material.isMeshPhysicalNodeMaterial) {
				...
				if (
					...
					material.specularColorNode.isNode
				) {
					this.addSlot(
						'fragment',
						new SlotNode({
	...
	buildFunctionCode(shaderNode) {
		...
		const code = `${this.getType(layout.type)} ${
			layout.name
		}( ${parameters.join(', ')} ) {
	...
	getAttributes(shaderStage) {
		...
		if (shaderStage === 'vertex') {
			...
			for (const attribute of attributes) {
				...
				if (/^(position|normal|uv[1-3]?)$/.test(attribute.name)) continue
	...
	getVaryings(shaderStage) {
		...
		if (shaderStage === 'vertex') {
			for (const varying of varyings) {
				snippet += `${
					varying.needsInterpolation ? 'varying' : '/*varying*/'

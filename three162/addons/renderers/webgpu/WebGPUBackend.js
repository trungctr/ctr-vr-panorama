...
class WebGPUBackend extends Backend {
	...
	beginRender(renderContext) {
		...
		if (occlusionQueryCount > 0) {
			...
			renderContextData.occlusionQueryObjects = new Array(
				occlusionQueryCount
			)
		...
		if (contextData.occlusionQuerySet !== undefined) {
			...
			if (lastObject !== object) {
				...
				if (object.occlusionTest === true) {
					...
					contextData.occlusionQueryObjects[
						contextData.occlusionQueryIndex
					] = object
				}
	...
	initTimestampQuery(renderContext, descriptor) {
		if (
			!this.hasFeature(GPUFeatureName.TimestampQuery) ||
			!this.trackTimestamp
		)
			return
	...
	async resolveTimestampAsync(renderContext, type = 'render') {
		...
		if (currentTimestampQueryBuffer) {
			...
			const times = new BigUint64Array(
				currentTimestampQueryBuffer.getMappedRange()
			)
	...
	hasFeature(name) {
		if (!this.adapter) {
			console.warn(
				'WebGPUBackend: WebGPU adapter has not been initialized yet. Please use detectSupportAsync instead'
			)

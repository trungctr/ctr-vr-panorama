...
`),
	mod_float: new CodeNode(
		'fn threejs_mod_float( x : f32, y : f32 ) -> f32 { return x - y * floor( x / y ); }'
	),
	mod_vec2: new CodeNode(
		'fn threejs_mod_vec2( x : vec2f, y : vec2f ) -> vec2f { return x - y * floor( x / y ); }'
	),
	mod_vec3: new CodeNode(
		'fn threejs_mod_vec3( x : vec3f, y : vec3f ) -> vec3f { return x - y * floor( x / y ); }'
	),
	mod_vec4: new CodeNode(
		'fn threejs_mod_vec4( x : vec4f, y : vec4f ) -> vec4f { return x - y * floor( x / y ); }'
	),
	equals_bool: new CodeNode(
		'fn threejs_equals_bool( a : bool, b : bool ) -> bool { return a == b; }'
	),
...
class WGSLNodeBuilder extends NodeBuilder {
	...
	getUniformFromNode(node, type, shaderStage, name = null) {
		const uniformNode = super.getUniformFromNode(
			node,
			type,
			shaderStage,
			name
	...
	buildFunctionCode(shaderNode) {
		...
		const code = `fn ${layout.name}( ${parameters.join(
			', '
...
${flowData.code}
	...
	getAttributes(shaderStage) {
		...
		if (shaderStage === 'vertex' || shaderStage === 'compute') {
			...
			for (
				let index = 0, length = attributes.length;
				index < length;
				index++
			) {

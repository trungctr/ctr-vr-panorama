...
class WebGPUTextureUtils {
	...
	createSampler(texture) {
		...
		if (texture.isDepthTexture && texture.compareFunction !== null) {
			samplerDescriptorGPU.compare =
				_compareToWebGPU[texture.compareFunction]
		}
	...
	createTexture(texture, options = {}) {
		...
		const format =
			texture.internalFormat || getFormat(texture, backend.device)

		let sampleCount =
			options.sampleCount !== undefined ? options.sampleCount : 1
		...
		if (texture.isRenderTargetTexture && sampleCount > 1) {
			const msaaTextureDescriptorGPU = Object.assign(
				{},
				textureDescriptorGPU
			)

			msaaTextureDescriptorGPU.label =

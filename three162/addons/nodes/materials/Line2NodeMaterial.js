			...
			if (useWorldUnits) {
				...
				worldPos.addAssign(
					vec4(
						positionGeometry.x
							.lessThan(0.0)
							.cond(worldUp.mul(hw), worldUp.mul(hw).negate()),
						0
					)
				)
				...
				if (!useDash) {
					...
					worldPos.addAssign(
						vec4(
							positionGeometry.y
								.lessThan(0.5)
								.cond(worldDir.mul(hw).negate(), worldDir.mul(hw)),
							0
						)
					)
				...
				clipPose.assign(
					positionGeometry.y.lessThan(0.5).cond(ndcStart, ndcEnd)
				)
				...
				// sign flip
				offset.assign(
					positionGeometry.x.lessThan(0.0).cond(offset.negate(), offset)
				)
				...
				// select end
				clip.assign(
					positionGeometry.y.lessThan(0.5).cond(clipStart, clipEnd)
				)
		...
		this.fragmentNode = tslFn(() => {
			...
			if (useDash) {
				...
				const dashSizeNode = this.dashSizeNode
					...
					: materialLineDashSize
				const gapSizeNode = this.dashSizeNode
					? float(this.dashGapNode)
					: materialLineGapSize
				...
				const lineDistance = positionGeometry.y
					...
					.cond(
						...
						materialLineScale.mul(instanceDistanceEnd)
					)

				const vLineDistance = varying(
					lineDistance.add(materialLineDashOffset)
				)
			...
			if (useWorldUnits) {
				...
				if (useAlphaToCoverage) {
					...
					const b = vUv.y
						.greaterThan(0.0)
						.cond(vUv.y.sub(1.0), vUv.y.add(1.0))
			...
			if (this.lineColorNode) {
				...
				if (useColor) {
					...
					const instanceColor = positionGeometry.y
						.lessThan(0.5)
						.cond(instanceColorStart, instanceColorEnd)

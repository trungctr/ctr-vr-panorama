...
} from '../materials/NodeMaterial.js'
import {
	NodeUpdateType,
	defaultBuildStages,
	shaderStages
...
class NodeBuilder {
	...
	generateConst(type, value = null) {
		...
		if (typeLength === 2) {
			return `${this.getType(type)}( ${generateConst(
				value.x
			...
			return `${this.getType(type)}( ${generateConst(
				value.x
			)}, ${generateConst(value.y)}, ${generateConst(
				value.z
			)}, ${generateConst(value.w)} )`
		} else if (
			typeLength > 4 &&
			value &&
	...
	getBufferAttributeFromNode(node, type) {
		...
		if (bufferAttribute === undefined) {
			...
			bufferAttribute = new NodeAttribute(
				'nodeAttribute' + index,
				type,
				node
			)
	...
	getUniformFromNode(node, type, shaderStage = this.shaderStage, name = null) {
		...
		if (nodeUniform === undefined) {
			...
			nodeUniform = new NodeUniform(
				name || 'nodeUniform' + index,
				type,
				node
			)
		...
		if (fromTypeLength > toTypeLength) {
			return this.format(
				...
				this.getTypeFromLength(
					toTypeLength,
					this.getComponentType(fromType)
				),
				toType
			)
		}

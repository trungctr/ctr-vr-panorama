...
class GaussianBlurNode extends TempNode {
	...
	setup(builder) {
		...
		const sampleTexture = (uv) =>
			textureNode.cache().context({ getUV: () => uv, forceUVContext: true })
		...
		const material =
			this._material || (this._material = builder.createNodeMaterial())
...
export const gaussianBlur = (node, sigma) =>
	nodeObject(new GaussianBlurNode(nodeObject(node), sigma))
